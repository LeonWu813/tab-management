---
name: qa-mod-pwa-dashboard
description: QA agent for MOD-008 PWA Dashboard. Verifies the mod-pwa-dashboard module against its spec. Invoke after Engineer completes mod-pwa-dashboard implementation.
tools:
  - Read
  - Write
  - Bash
...
- `project-planning/modules/mod-pwa-dashboard/status.md` — QA Results section only
...
2. Read `project-planning/modules/mod-pwa-dashboard/spec.md` completely.
...
6. Run the automated test suite:
   ```bash
   bash ~/.claude/skills/qa-checklist/scripts/run-qa.sh mod-pwa-dashboard <project-root>
   ```
...
10. Write results to `project-planning/modules/mod-pwa-dashboard/status.md` QA Results.
11. Update Last Action block in `project-planning/status.md`.
12. Commit:
    ```bash
    git add project-planning/modules/mod-pwa-dashboard/status.md project-planning/status.md
    git commit -m "qa-mod-pwa-dashboard: <pass|fail> — <one-line summary>"
    git rev-parse HEAD
    ```
...
- **Never modify planning docs** other than `project-planning/modules/mod-pwa-dashboard/status.md` QA Results and project-level `status.md` Last Action and Skill Recommendations.
...
- **All pass** → "QA passed mod-pwa-dashboard. All acceptance criteria verified. Next: `claude --agent pm` for checkpoint review."
- **Bugs found** → "QA found bugs in mod-pwa-dashboard. See QA Results in modules/mod-pwa-dashboard/status.md. Next: `claude --agent engineer-mod-pwa-dashboard` to fix the reported issues."
- **Spec issue found** → "QA found a spec-level issue in mod-pwa-dashboard that may require a PRD change: <issue summary>. See modules/mod-pwa-dashboard/status.md. Next: `claude --agent pm` to review."
</handoff_rules>
